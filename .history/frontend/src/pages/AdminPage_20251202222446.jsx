...
const AdminPage = () => {
  ...
  const refreshAccessToken = async () => {
    ...
    try {
      const response = await fetch(`${window.location.origin}/api/v1/contact/refresh`, {
        method: "POST",
  ...
  const fetchUsers = async () => {
    try {
      ...
      const res = await fetch(`${window.location.origin}/api/v1/contact/all`, {
        headers: {
      ...
      if (res.status === 401) {
        ...
        // Retry request
        const retry = await fetch(`${window.location.origin}/api/v1/contact/all`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
